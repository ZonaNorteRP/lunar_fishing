const FishingApp = {
    ...
    createItemCard(item, type, index, playerLevel) {
        ...
        card.innerHTML = `
            ...
            <button class="btn-buy" ${isLocked ? 'disabled' : ''} onclick="FishingApp.buyItem(event, '${type}', ${index})">COMPRAR</button>
    ...
    async buyItem(event, type, index) {
        if (this.buyCooldown) return;
